...
/**
 * FluidBackground — campo di flusso a rumore di Perlin (dal componente 21st.dev),
 * ricolorato WhyChat: particelle crimson/ember che scivolano sul void come una
 * corrente. Fisso a tutto schermo, dietro tutto, conteggio ridotto per girare
 * liscio anche su GPU integrate. Si spegne con prefers-reduced-motion.
 */
...
interface P {
  ...
  life: number;
  maxLife: number;
  hue: number; // 0 crimson … 1 ember
}

export default function FluidBackground({ count = 760 }: { count?: number }) {
  ...
  useEffect(() => {
    ...
    let raf = 0;
    const tick = () => {
      // scia su void: lascia tracce morbide
      ctx.fillStyle = "rgba(10,9,8,0.10)";
      ...
      for (const p of particles) {
        ...
        const n = noise.n3(p.x * 0.0024, p.y * 0.0024, time);
        const a = n * Math.PI * 4;
        p.x += Math.cos(a) * 1.6;
        p.y += Math.sin(a) * 1.6;
        if (p.x < 0) p.x = W();
