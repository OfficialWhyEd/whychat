...
/**
 * ProgressiveFluxLoader — barra di caricamento con banda di luce e label di fase
 * che entra in 3D. Brandizzata (flux ember→signal). Usata per loading di alto
 * livello sulla versione PC. Adattata da framer-motion.
 */
...
const DEFAULT_PHASES: ProgressiveFluxPhase[] = [
  { at: 0, label: "avvio" },
  { at: 25, label: "raccolgo" },
  { at: 55, label: "elaboro" },
  { at: 80, label: "ci siamo quasi" },
  { at: 100, label: "fatto" },
];

const FLUX_FROM = "var(--flux-from, #c94b25)";
const FLUX_TO = "var(--flux-to, #f0a36a)";
...
const BAR_SHADOW = `0 0 18px color-mix(in oklab, ${FLUX_FROM} 55%, transparent), 0 0 32px color-mix(in oklab, ${FLUX_TO} 40%, transparent), inset 0 1.5px 0 rgba(255,255,255,0.4)`;
const SHEEN_GRADIENT = "linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.5) 50%, transparent 100%)";
...
function FluxLabel({ label, reduced, className }: { label: string; reduced: boolean; className?: string }) {
  const base = cn(
    "absolute inset-0 flex items-center justify-center text-center text-2xl font-medium tracking-tight text-faint sm:text-3xl",
    className,
  );
  if (reduced)
    return (
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        exit={{ opacity: 0, z: 220, scale: 1.35, filter: "blur(10px)", transition: { duration: 0.45, ease: [0.7, 0, 0.84, 0] } }}
...
export function ProgressiveFluxLoader({
  ...
  return (
    <div className={cn("mx-auto flex w-full max-w-md flex-col items-center gap-7", className)}>
      {showLabel && (
        <div className="relative h-14 w-full select-none" style={reduced ? undefined : { perspective: "1000px" }}>
          <FluxLabel label={label} reduced={reduced} className={textClassName} />
        </div>
      )}
      <div
        className={cn("relative h-4 w-full overflow-hidden rounded-full bg-[rgba(242,239,233,0.08)]", barClassName)}
        role="progressbar"
        aria-valuemin={0}
        aria-valuemax={100}
